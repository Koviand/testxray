...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --panel-mode) PANEL_MODE=1; shift ;;
        -h|--help)
            echo "Usage: $0 [--panel-mode] <domain>"
            exit 0
            ;;
        *)
            DOMAIN="$1"
...
if [ "$LOCAL_IP" != "$DNS_IP" ]; then
    ...
    if [ "${AUTOXRAY_NONINTERACTIVE:-0}" = "1" ]; then
        echo -e "${RED}AUTOXRAY_NONINTERACTIVE=1 — прерывание.${NC}"
        exit 1
    fi
...
if [ "$PANEL_MODE" -eq 1 ]; then
  ...
  CERTBOT_DEPLOY="systemctl reload nginx; cp /etc/letsencrypt/live/$DOMAIN/fullchain.pem /var/lib/xray/cert/fullchain.pem; cp /etc/letsencrypt/live/$DOMAIN/privkey.pem /var/lib/xray/cert/privkey.pem; chmod 744 /var/lib/xray/cert/privkey.pem; chmod 744 /var/lib/xray/cert/fullchain.pem; bash $CERTBOT_HOOK"
else
...
mkdir -p /etc/autoXRAY
cat > /etc/autoXRAY/panel-metadata.json <<METADATA
{
  "version": 1,
  "managed_by": "testxray",
  "domain": "${DOMAIN}",
  "uuid": "${xray_uuid_vrv}",
  "private_key": "${xray_privateKey_vrv}",
  "public_key": "${xray_publicKey_vrv}",
  "short_id": "${xray_shortIds_vrv}",
  "path_xhttp": "${path_xhttp}",
  "path_subpage": "${path_subpage}",
  "socks_user": "${socksUser}",
  "socks_pass": "${socksPasw}",
  "inbound_tags": [
    "vsWSinternal",
    "vsXHTTPrty",
    "vsXHTTPtls",
...
METADATA
chmod 600 /etc/autoXRAY/panel-metadata.json
...
if [ "$PANEL_MODE" -eq 1 ]; then
  echo -e "${GRN}Panel mode: пропуск systemctl restart xray (управляет 3x-ui).${NC}"
else
...
if [ "$PANEL_MODE" -eq 1 ]; then
    if systemctl is-active --quiet x-ui 2>/dev/null; then
        echo -e "3x-ui: ${GRN}RUNNING${NC} (panel mode)"
    else
        echo -e "3x-ui: ${YEL}NOT RUNNING YET${NC} (install panel next)"
    fi
